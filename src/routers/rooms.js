...
router.post("/api/rooms", auth, async (req, res, next) => {
  try {
...
router.post("/api/rooms/add/:id/:room", auth, async (req, res, next) => {
  try {
...
router.post("/api/rooms/remove/:id/:room", auth, async (req, res, next) => {
  try {
    const room = await Room.findOne({ _id: req.params.room });
...
router.post("/api/rooms/leave/:room", auth, async (req, res, next) => {
  try {
    ...
    if (room.owner.equals(req.user.id)) {
      throw createError(
        400,
        "The owner can not leave the group.The owner needs to delete the room"
      );
    } else {
      room.users = room.users.filter((el) => {
...
router.delete("/api/rooms/:room", auth, async (req, res, next) => {
  try {
    const room = await Room.findOne({ _id: req.params.room });
...
router.get("/api/rooms/me", auth, async (req, res, next) => {
  try {
